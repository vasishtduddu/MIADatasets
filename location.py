...
def configure(raw_path):
    ...
    location_path: Path = raw_path / "Location30"

    create_dir_if_doesnt_exist(location_path)

    downloaded_location: Path = location_path / "dataset_location.tgz"

    wget.download("https://raw.githubusercontent.com/vasishtduddu/MIADatasets/main/dataset_location.tgz", str(downloaded_location))

    tar = tarfile.open(downloaded_location)
    tar.extractall(location_path)
    tar.close()
    (location_path / "bangkok").replace(location_path / "location")
...
def prepare_location_data(datapath):

    path = datapath / "Location30" / "location"
    ...
    train_data = X[:2500]
    test_data = X[2500:2500+2500]

    train_label = Y[:2500]
    test_label = Y[2500:2500+2500]
...
def main(args: argparse.Namespace) -> None:
    ...
    num_classes = 30
    train_dataset, test_dataset = prepare_location_data(raw_path)
    model = LocationClassifier(num_classes=num_classes).to(args.device)
...
if __name__ == "__main__":
    args = handle_args()
    if (Path(args.raw_path) / "Location30").exists():
